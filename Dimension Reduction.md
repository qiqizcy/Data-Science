...
# INTRODUCTION
>**Why we need to reduce the dimentions of features?**
>  Sometimes we think more data can achieve better model, but are the present dimentions of features real?  For example, when the data has multicollinearity, the real dimentions of the data may smaller than the present dimentions.  Next, just condering the influence of single variable to the target variable may ignore the latent relation between variables. Last, high dimentions of features may increase the complexity of our model. To solve this problems, we need to reduce the dimentions before training model.

> **What does the article do?**
> I collect 14 methods of feature dimention reduction, and divide them to two catagories, One is called DROP which means the dimention reduction technique is to drop some variables directly, another is called GENERATE which means we use machine learning model to generate new variables that have smaller dimentions than the original data.
> To make these methods more structured, I list four aspects for every method. Such as **concept or procedure**, **python code**, **advantages and disadvantages**, **applying situations**.
>  
| DROP | GENERATE  |
|--|--|
...
**APPLYING SITUATION**  
PCA suits for linear problem and numeric variables which differentiated by their variance. And it also suits data that satisfies a Gaussian distribution.
```python
import pandas as pd
...
**CONCEPT**
Compared with PCA, LDA is projecting raw data into a low-dimensional space, and try to make the same labels of data aggregation and different labels of data dispersed. But LDA default two hypotheses:
 1. The original data is classified according to mean value.
 2. Different labels of data have same covariance matrix.  
 
 **ADVANTAGE**

 5. Fast calculating speed.
 6. Take full advantage of prior knowledge.

**DISADVANTAGE**

 1. If the number of feature is larger than the data size, the performance of LDA will be worse.
 2. Sometimes it causes overfit.

**APPLYING SITUATION**  
This method suits for data which differentiated by mean value. And it also suits data that satisfies a Gaussian distribution.(Same as PCA)
```python
import pandas as pd
...
**CONCEPT**  
MDS is aimed to project the points in a higher-dimensional coordinate into a lower-dimentional space, and keeping the similarity between the points as constant as possible.
```python
import numpy as np
def MDS(D,d):
	D = np.asarray(D)
	...
	return X
```
## 2.6. Equidistant Feature Mapping(ISOMAP)
**CONCEPT**  
...
## 2.7. t-SNE
**CONCEPT**  
Compared with PCA, t-SNE is not a linear dimention reduction technique, on the contrary, it's a nonlinear technique. And it projects n dimentions to 2 or 3 dimentions capturing complex manifold structure.
```python
import pandas as pd
import matplotlib.pyplot as plt
...
```
## 2.8. Uniform Manifold Approximation Projection(UMAP)
**CONCEPT**  
UMAP assums data are uniformly distributed in the manifold space, and it can approximate and project data to a lower dimention space.  
**PROCEDURE**
 1. learn the manifold structure from higher dimention space.
 2. denote the manifold structure in a lower dimention.
```python
import pandas as pd
import matplotlib.pyplot as plt
import umap
...
```
> 2.5 -2.8 are the methods which suit for non-linear data processing, and t-SNE is more visualized than ISOMAP, UMAP also suits for high dimentions data and faster than t-SNE. ALL the four methods  capture the manifold structure data.
